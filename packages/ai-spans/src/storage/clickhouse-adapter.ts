  ...
  async listObservations(filters: ObservationFilters = {}): Promise<ObservationListResult> {
    ...
    return {
      rows: rows.map((row) => ({
        ...
        observationType: String(row.observationType ?? 'unknown'),
  ...
  async getTrace(traceId: string): Promise<TraceDetail> {
    ...
    const rows = await this.client.queryJsonEachRow<QueryRow>(`
      SELECT
        ...
        tool_name AS toolName,
        tool_id AS toolId,
        tool_input_json AS toolInputJson,
        tool_output_json AS toolOutputJson,
        tool_error AS toolError,
        start_time AS startTime,
        end_time AS endTime,
        duration_ms AS durationMs,
        status_code AS statusCode,
    ...
    return {
      ...
      spans: rows.map((row) => ({
        ...
        observationType: String(row.observationType ?? 'unknown'),
        toolName: asString(row.toolName),
        toolId: asString(row.toolId),
        toolInputJson: asString(row.toolInputJson),
        toolOutputJson: asString(row.toolOutputJson),
        toolError: asString(row.toolError),
        startTime: String(row.startTime),
        endTime: String(row.endTime),
        durationMs: asNumber(row.durationMs),
        statusCode: asNumber(row.statusCode),
